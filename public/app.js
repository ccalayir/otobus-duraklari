...
async function duraklariYukle() {
    try {
        const cevap = await fetch('http://127.0.0.1:3000/app/stops/all');
...
aramaButonu.addEventListener("click", async () => {
    ...
    try {
        const response = await fetch(`http://127.0.0.1:3000/app/stops/${id}`);
        ...
        if (hatlarDizisi.length > 0) {
            hatlarDizisi.forEach(async (hat) => {
                try {
                    const oCevap = await fetch(`http://127.0.0.1:3000/app/stops/buses/${hat}`);
